...
FTM_INT	FTM_OM_EP_CLASS_seeker(const FTM_VOID_PTR pElement, const FTM_VOID_PTR pIndicator);
FTM_INT	FTM_OM_EP_CLASS_comparator(const FTM_VOID_PTR pElement1, const FTM_VOID_PTR pElement2);

FTM_RET	FTM_OM_EP_CLASS_init(FTM_VOID)
{
	FTM_RET	nRet;
	...
	FTM_LIST_setSeeker(&xEPList, FTM_OM_EP_CLASS_seeker);
	FTM_LIST_setComparator(&xEPList, FTM_OM_EP_CLASS_comparator);

	return	nRet;
}

FTM_RET FTM_OM_EP_CLASS_final(FTM_VOID)
{
	FTM_EP_CLASS_PTR	pEP;

	while(FTM_LIST_getAt(&xEPList, 0, (FTM_VOID_PTR _PTR_)&pEP) == FTM_RET_OK)
	{
		FTM_OM_EP_CLASS_destroy(pEP);
	}
...
FTM_RET	FTM_OM_EP_CLASS_create(FTM_EP_CLASS_PTR pInfo)
{
...
FTM_RET	FTM_OM_EP_CLASS_destroy(FTM_EP_CLASS_PTR	pEPClass)
{
	FTM_RET	nRet;
...
FTM_RET	FTM_OM_EP_CLASS_count(FTM_ULONG_PTR pulCount)
{
	return	FTM_LIST_count(&xEPList, pulCount);
}

FTM_RET FTM_OM_EP_CLASS_get(FTM_EP_TYPE xType, FTM_EP_CLASS_PTR _PTR_ ppEP)
{
	return	FTM_LIST_get(&xEPList, &xType, (FTM_VOID_PTR _PTR_)ppEP);
}

FTM_RET FTM_OM_EP_CLASS_getAt(FTM_ULONG ulIndex, FTM_EP_CLASS_PTR _PTR_ ppEP)
{
