...
static FTM_RET FTM_OM_CLIENT_request
(
	FTM_OM_CLIENT_PTR		pClient, 
	FTM_OM_REQ_PARAMS_PTR		pReq,
	FTM_ULONG				ulReqLen,
	FTM_OM_RESP_PARAMS_PTR	pRespBuff,
	FTM_ULONG				ulRespBuffLen,
	FTM_ULONG_PTR			pulRespLen
);

static FTM_RET FTM_OM_CLIENT_TRANS_init
(
	FTM_OM_CLIENT_TRANS_PTR			pTrans,
	FTM_OM_REQ_PARAMS_PTR		pReq,
	FTM_ULONG				ulReqLen,
...
FTM_RET FTM_OM_CLIENT_TRANS_final
(
	FTM_OM_CLIENT_TRANS_PTR			pTrans
);

static FTM_VOID_PTR	FTM_OM_CLIENT_process(FTM_VOID_PTR pData);
static FTM_BOOL 	FTM_OM_CLIENT_transSeeker(const FTM_VOID_PTR pElement, const FTM_VOID_PTR pIndicator);
...
)
{
	...
	memset(pClient, 0, sizeof(FTM_OM_CLIENT));
	...
	FTM_LIST_setSeeker(&pClient->xTransList, FTM_OM_CLIENT_transSeeker);
...
)
{
	...
	pClient = (FTM_OM_CLIENT_PTR)FTM_MEM_malloc(sizeof(FTM_OM_CLIENT));
	if (pClient == NULL)
	{
...
)
{
	...
	if (pthread_create(&pClient->xThread, NULL, FTM_OM_CLIENT_process, pClient) < 0)
	{
...
FTM_VOID_PTR	FTM_OM_CLIENT_process(FTM_VOID_PTR pData)
{
	ASSERT(pData != NULL);
	
	FTM_RET			xRet;
	FTM_OM_CLIENT_PTR	pClient = (FTM_OM_CLIENT_PTR)pData;
	...
	while(!pClient->bStop)
	{
		...
		{
			xRet = FTM_OM_CLIENT_connect(pClient);
			if (xRet != FTM_RET_OK)
			{
		...
		{
			FTM_BYTE				pBuff[2048];
			FTM_OM_RESP_PARAMS_PTR 	pResp = (FTM_OM_RESP_PARAMS_PTR)pBuff;
			...
			if (nLen > 0)
			{
				...
				{
					FTM_OM_RESP_NOTIFY_PARAMS_PTR pNotify = (FTM_OM_RESP_NOTIFY_PARAMS_PTR)pResp;
	...
	FTM_OM_CLIENT_disconnect(pClient);
...
)
{
	...
	memcpy(&pClient->xConfig, pConfig, sizeof(FTM_OM_CLIENT_CONFIG));
...
)
{
	...
	FTM_RET							nRet;
	FTM_OM_REQ_NODE_CREATE_PARAMS		xReq;
	FTM_OM_RESP_NODE_CREATE_PARAMS	xResp;
	FTM_ULONG						ulRespLen;
...
{
	FTM_RET							nRet;
	FTM_OM_REQ_NODE_DESTROY_PARAMS	xReq;
	FTM_OM_RESP_NODE_DESTROY_PARAMS	xResp;
	FTM_ULONG						ulRespLen;
	...
	xReq.xCmd 	=	FTM_OM_CMD_NODE_DESTROY;
	xReq.ulLen	=	sizeof(xReq);
...
{
	FTM_RET						nRet;
	FTM_OM_REQ_NODE_COUNT_PARAMS	xReq;
	FTM_OM_RESP_NODE_COUNT_PARAMS	xResp;
	FTM_ULONG					ulRespLen;
	...
	xReq.xCmd 	=	FTM_OM_CMD_NODE_COUNT;
	xReq.ulLen	=	sizeof(xReq);
...
{
	FTM_RET							nRet;
	FTM_OM_REQ_NODE_GET_AT_PARAMS		xReq;
	FTM_OM_RESP_NODE_GET_AT_PARAMS	xResp;
	FTM_ULONG						ulRespLen;
	...
	xReq.xCmd 	=	FTM_OM_CMD_NODE_GET_AT;
	xReq.ulLen	=	sizeof(xReq);
...
{
	FTM_RET							nRet;
	FTM_OM_REQ_NODE_GET_PARAMS		xReq;
	FTM_OM_RESP_NODE_GET_PARAMS	xResp;
	FTM_ULONG					ulRespLen;
...
{
	FTM_RET						nRet;
	FTM_OM_REQ_EP_CREATE_PARAMS	xReq;
	FTM_OM_RESP_EP_CREATE_PARAMS	xResp;
	FTM_ULONG					ulRespLen;
	...
	xReq.xCmd	=	FTM_OM_CMD_EP_CREATE;
	xReq.ulLen	=	sizeof(xReq);
...
{
	FTM_RET					nRet;
	FTM_OM_REQ_EP_DESTROY_PARAMS	xReq;
	FTM_OM_RESP_EP_DESTROY_PARAMS	xResp;
	FTM_ULONG					ulRespLen;
	...
	xReq.xCmd	=	FTM_OM_CMD_EP_DESTROY;
	xReq.ulLen	=	sizeof(xReq);
	xReq.xEPID	=	xEPID;
...
{
	FTM_RET						nRet;
	FTM_OM_REQ_EP_COUNT_PARAMS	xReq;
	FTM_OM_RESP_EP_COUNT_PARAMS	xResp;
	FTM_ULONG					ulRespLen;
...
{
	FTM_RET	nRet;
	FTM_OM_REQ_EP_GET_LIST_PARAMS		xReq;
	...
	nRespSize = sizeof(FTM_OM_RESP_EP_GET_LIST_PARAMS) + sizeof(FTM_EP_ID) * ulMaxCount;
	pResp = (FTM_OM_RESP_EP_GET_LIST_PARAMS_PTR)FTM_MEM_malloc(nRespSize);
	if (pResp == NULL)
	{
		return	FTM_RET_NOT_ENOUGH_MEMORY;
	}

	xReq.xCmd		=	FTM_OM_CMD_EP_GET_LIST;
	xReq.ulLen		=	sizeof(xReq);
	xReq.xType		=	xType;
...
{
	FTM_RET						nRet;
	FTM_OM_REQ_EP_GET_PARAMS		xReq;
	FTM_OM_RESP_EP_GET_PARAMS		xResp;
	FTM_ULONG					ulRespLen;
...
{
	FTM_RET						nRet;
	FTM_OM_REQ_EP_GET_AT_PARAMS	xReq;
	FTM_OM_RESP_EP_GET_AT_PARAMS	xResp;
	FTM_ULONG					ulRespLen;
	...
	xReq.xCmd	=	FTM_OM_CMD_EP_GET_AT;
	xReq.ulLen	=	sizeof(xReq);
	xReq.ulIndex=	ulIndex;
...
{
	FTM_RET							nRet;
	FTM_OM_REQ_EP_DATA_ADD_PARAMS		xReq;
	FTM_OM_RESP_EP_DATA_ADD_PARAMS	xResp;
	FTM_ULONG						ulRespLen;
	...
	xReq.xCmd	=	FTM_OM_CMD_EP_DATA_ADD;
 ...
 *
 *****************************************************************/
FTM_RET	FTM_OM_CLIENT_EP_DATA_info
(
	FTM_OM_CLIENT_PTR		pClient,
	FTM_EP_ID				xEPID,
...
{
	FTM_RET							nRet;
	FTM_OM_REQ_EP_DATA_INFO_PARAMS	xReq;
	FTM_OM_RESP_EP_DATA_INFO_PARAMS	xResp;
	FTM_ULONG						ulRespLen;
	...
	xReq.xCmd		=	FTM_OM_CMD_EP_DATA_INFO;
	xReq.ulLen		=	sizeof(xReq);
	xReq.xEPID		=	xEPID;
...
{
	FTM_RET								nRet;
	FTM_OM_REQ_EP_DATA_GET_LAST_PARAMS	xReq;
	FTM_OM_RESP_EP_DATA_GET_LAST_PARAMS	xResp;
	FTM_ULONG						ulRespLen;
	...
	xReq.xCmd	=	FTM_OM_CMD_EP_DATA_GET_LAST;
	xReq.ulLen	=	sizeof(xReq);
	xReq.xEPID	=	xEPID;
...
{
	FTM_RET									nRet;
	FTM_OM_REQ_EP_DATA_GET_LIST_PARAMS		xReq;
	FTM_ULONG								nRespSize = 0;
	FTM_OM_RESP_EP_DATA_GET_LIST_PARAMS_PTR	pResp = NULL;
	FTM_ULONG								ulRespLen;
	...
	while(!bStop)
	{
		...
		nRespSize = sizeof(FTM_OM_RESP_EP_DATA_GET_LIST_PARAMS) + sizeof(FTM_EP_DATA) * ulReqCount;
		pResp = (FTM_OM_RESP_EP_DATA_GET_LIST_PARAMS_PTR)FTM_MEM_malloc(nRespSize);
		if (pResp == NULL)
		{
			return	FTM_RET_NOT_ENOUGH_MEMORY;
		}
	
		xReq.xCmd		=	FTM_OM_CMD_EP_DATA_GET_LIST;
		xReq.ulLen		=	sizeof(xReq);
		xReq.xEPID		=	xEPID;
...
{
	FTM_RET							nRet;
	FTM_OM_REQ_EP_DATA_DEL_PARAMS		xReq;
	FTM_OM_RESP_EP_DATA_DEL_PARAMS	xResp;
	FTM_ULONG						ulRespLen;
	...
	xReq.xCmd		=	FTM_OM_CMD_EP_DATA_DEL;
	xReq.nLen		=	sizeof(xReq);
	xReq.xEPID		=	xEPID;
	xReq.nIndex		=	nIndex;
	xReq.nCount		=	nCount;
...
{
	FTM_RET						nRet;
	FTM_OM_REQ_EP_DATA_COUNT_PARAMS	xReq;
	FTM_OM_RESP_EP_DATA_COUNT_PARAMS	xResp;
	FTM_ULONG						ulRespLen;
	...
	xReq.xCmd		=	FTM_OM_CMD_EP_DATA_COUNT;
	xReq.ulLen		=	sizeof(xReq);
	xReq.xEPID		=	xEPID;
...
)
{
	...
	FTM_OM_CLIENT_TRANS_init(&xTrans, pReq, ulReqLen, pResp, ulRespLen);
	...
	FTM_OM_CLIENT_TRANS_final(&xTrans);

	return	xRet;	
}


FTM_BOOL FTM_OM_CLIENT_transSeeker(const FTM_VOID_PTR pElement, const FTM_VOID_PTR pIndicator)
{
	ASSERT(pElement != NULL);
	ASSERT(pIndicator != NULL);

	FTM_OM_CLIENT_TRANS_PTR	pTrans = (FTM_OM_CLIENT_TRANS_PTR)pElement;
	FTM_ULONG_PTR	pReqID = (FTM_ULONG_PTR)pIndicator;

	return	(pTrans->pReq->ulReqID == *pReqID);
}

FTM_RET FTM_OM_CLIENT_TRANS_init
(
	FTM_OM_CLIENT_TRANS_PTR			pTrans,
	FTM_OM_REQ_PARAMS_PTR		pReq,
	FTM_ULONG				ulReqLen,
