...
FTM_VOID_PTR	FTM_OM_SNMPC_process(FTM_VOID_PTR pData);
...
)
{
	...
	init_agent("ftm_om:snmpc");
	init_snmp("ftm_om:snmpc");

	memset(pClient, 0, sizeof(FTM_OM_SNMPC));
...
FTM_RET	FTM_OM_SNMPC_final(FTM_OM_SNMPC_PTR pClient)
{
	ASSERT(pClient != NULL);
...
FTM_RET FTM_OM_SNMPC_start(FTM_OM_SNMPC_PTR pClient)
{
	ASSERT(pClient != NULL);

	FTM_INT	nRet;

	nRet = pthread_create(&pClient->xPThread, NULL, FTM_OM_SNMPC_process, pClient);
	if (nRet != 0)
	{
		switch(nRet)
		{
...
FTM_RET	FTM_OM_SNMPC_stop(FTM_OM_SNMPC_PTR pClient)
{
	ASSERT(pClient != NULL);
...
FTM_VOID_PTR	FTM_OM_SNMPC_process(FTM_VOID_PTR pData)
{
	...
	FTM_OM_SNMPC_PTR	pClient = (FTM_OM_SNMPC_PTR)pData;
...
FTM_RET FTM_OM_SNMPC_loadFromFile(FTM_OM_SNMPC_PTR pClient, FTM_CHAR_PTR pFileName)
{
	ASSERT(pClient != NULL);
...
FTM_RET FTM_OM_SNMPC_showConfig(FTM_OM_SNMPC_PTR pClient)
{
	ASSERT(pClient != NULL);
...
FTM_RET	FTM_OM_SNMPC_setServiceCallback(FTM_OM_SNMPC_PTR pClient, FTM_OM_SERVICE_ID xServiceID, FTM_OM_SERVICE_CALLBACK fServiceCB)
{
	ASSERT(pClient != NULL);
...
FTM_RET	FTM_OM_SNMPC_getEPData(FTM_OM_NODE_SNMPC_PTR pNode, FTM_OM_EP_PTR pEP, FTM_EP_DATA_PTR pData)
{
	ASSERT(pNode != NULL);
	ASSERT(pEP != NULL);
	ASSERT(pData != NULL);
					...
					case	6:
						{
							...
							xRet = FTM_OM_EP_getDataType(pEP, &xDataType);
							if (xRet != FTM_RET_OK)
							{
								break;
							}
...
FTM_RET	FTM_OM_SNMPC_setEPData(FTM_OM_NODE_SNMPC_PTR pNode, FTM_OM_EP_PTR pEP, FTM_EP_DATA_PTR pData)
{
	ASSERT(pNode != NULL);
	ASSERT(pEP != NULL);
	ASSERT(pData != NULL);
...
FTM_RET		FTM_OM_SNMPC_getOID(FTM_CHAR_PTR pInput, oid *pOID, size_t	*pnOIDLen)
{
