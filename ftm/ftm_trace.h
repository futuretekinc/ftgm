...
#define	ERROR2(code, format, ...)\
		{\
			if (format != NULL)\
			{\
				FTM_TRACE_out2(FTM_TRACE_LEVEL_ERROR, __func__, __LINE__, code, format, ## __VA_ARGS__);\
			}\
			else\
			{\
				FTM_TRACE_out2(FTM_TRACE_LEVEL_ERROR, __func__, __LINE__, code, "", 0);\
			}\
		}
