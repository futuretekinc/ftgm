...
FTM_INT main(int nArgc , char *pArgv[])
{
	...
	xRet = FTOM_NET_CLIENT_create("client", &pClient);
	if (xRet != FTM_RET_OK)
	{
	...
	if (pConfig != NULL)
	{
		xRet = FTOM_NET_CLIENT_CONFIG_load(pClient, pConfig);
		if (xRet != FTM_RET_OK)                                                                                                                                                                                           
		{
	...
	FTOM_NET_CLIENT_start(pClient);
	FTOM_NET_CLIENT_setNotifyCB(pClient, FTOM_CLIENT_subscriber, pClient);
	 
	FTM_SHELL_run(&xShell);
	FTM_SHELL_final(&xShell);

	FTOM_NET_CLIENT_destroy(&pClient);
