...
FTM_RET	FTNM_getMACAddress(FTM_CHAR_PTR pIFName, FTM_UINT8	pMAC[6]);
...
FTM_RET	FTNM_getDefaultDeviceID(FTM_CHAR	pDID[FTM_DID_LEN + 1])
{
	FTM_INT		i;
	FTM_RET		xRet;
	FTM_UINT8	pMAC[6];
	...
	{
		xRet = FTNM_getMACAddress(pIFList[i], pMAC);	
		if (xRet == FTM_RET_OK)
		{
