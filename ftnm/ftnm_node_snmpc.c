...
FTM_RET			FTNM_NODE_SNMPC_setEPData(FTNM_NODE_SNMPC_PTR pNode, FTNM_EP_PTR pEP, FTM_EP_DATA_PTR pData);
...
FTM_RET	FTNM_NODE_SNMPC_create(FTM_NODE_PTR pInfo, FTNM_NODE_PTR _PTR_ ppNode)
{
	...
	pNode->xCommon.fSetEPData=(FTNM_NODE_SET_EP_DATA)FTNM_NODE_SNMPC_setEPData;
...
FTM_RET	FTNM_NODE_SNMPC_setEPData(FTNM_NODE_SNMPC_PTR pNode, FTNM_EP_PTR pEP, FTM_EP_DATA_PTR pData)
{
	return	FTNM_SNMPC_setEPData(pNode, pEP, pData);
}
