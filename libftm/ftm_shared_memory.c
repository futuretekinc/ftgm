...
)
{
	...
	clock_gettime(CLOCK_REALTIME, &xTimeout);
	xTimeout.tv_sec = xTimeout.tv_sec + (xTimeout.tv_nsec / 1000 + ulTimeout) / 1000000;
	xTimeout.tv_nsec = ((xTimeout.tv_nsec / 1000 + ulTimeout) % 1000000) * 1000 + xTimeout.tv_nsec % 1000;
	...
	if (nRet != 0)
	{
		ERROR("Respose timeout[Timeout = %lu ms].\n", ulTimeout / 1000);
		xRet = FTM_RET_TIMEOUT;	
		goto error;
	}
